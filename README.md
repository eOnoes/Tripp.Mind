...
## Tripp.Mind Production Quick Start

This repository includes the Tripp.Mind production overlay for SiYuan: a JWT-protected API gateway, Redis, event bridge, Telegram bot, Discord bot, backup service, Python SDK, and a single-file knowledge graph dashboard.

1. Review `.env`. `SIYUAN_ACCESS_AUTH_CODE` and `SIYUAN_API_TOKEN` are intentionally set to the same generated 32+ character token so the SiYuan kernel and gateway use one shared SiYuan API credential.
2. Start the core stack with `docker compose up -d`.
3. Start optional bots with `docker compose --profile telegram --profile discord up -d`.
4. Open SiYuan through the gateway at `http://localhost:3000`.
5. Open `dashboard/index.html`, set the gateway URL to `http://localhost:3000`, paste `SIYUAN_GATEWAY_TOKEN` from `.env`, and refresh.

### Environment Variables

| Variable | Required | Default | Purpose |
|---|---:|---|---|
| `SIYUAN_ACCESS_AUTH_CODE` | Yes | none | SiYuan lock/API auth code; must match `SIYUAN_API_TOKEN`. |
| `SIYUAN_API_TOKEN` | Yes | none | Token injected by the gateway when proxying to SiYuan; must match `SIYUAN_ACCESS_AUTH_CODE`. |
| `JWT_SECRET` | Yes | none | HMAC secret used to validate gateway JWTs. |
| `JWT_ISSUER` | No | empty | Expected JWT issuer when set. |
| `JWT_AUDIENCE` | No | empty | Expected JWT audience when set. |
| `SIYUAN_GATEWAY_TOKEN` | Yes for fleet services | none | JWT used by SDK, bots, dashboard, and backup service. |
| `SIYUAN_BOT_GATEWAY_TOKEN` | No | `SIYUAN_GATEWAY_TOKEN` | Bot-specific JWT override. |
| `SIYUAN_BACKUP_GATEWAY_TOKEN` | No | `SIYUAN_GATEWAY_TOKEN` | Backup-specific JWT override. |
| `GATEWAY_PORT` | No | `3000` | Host port for the API gateway. |
| `DASHBOARD_CORS_ORIGIN` | No | `*` | CORS origin for the static dashboard. |
| `EVENT_BRIDGE_PORT` | No | `3001` | Host/container port for the WebSocket event bridge. |
| `EVENT_BRIDGE_AUTH_TOKEN` | No | empty | Optional bearer/query token for event bridge clients. |
| `TELEGRAM_BOT_TOKEN` | Yes for Telegram profile | empty | Telegram bot token. |
| `TELEGRAM_ALLOWED_CHAT_IDS` | No | empty | Comma-separated allowlist; empty allows all chats. |
| `DISCORD_BOT_TOKEN` | Yes for Discord profile | empty | Discord bot token. |
| `DISCORD_APPLICATION_ID` | Yes for Discord profile | empty | Discord application ID for slash command registration. |
| `DISCORD_GUILD_ID` | No | empty | Optional guild-scoped command registration. |
| `SIYUAN_DEFAULT_NOTEBOOK` | No | first open notebook | Notebook ID used by bot `/remember` commands. |
| `BACKUP_CRON_SCHEDULE` | No | `0 2 * * *` | Backup schedule in cron format. |
| `BACKUP_RETENTION_COUNT` | No | `14` | Number of backup zip files to retain. |
| `BACKUP_RUN_ON_STARTUP` | No | `true` | Run one backup when the service starts. |

### Architecture

```text
Users / SDK / Dashboard / Bots
            |
            v
    JWT API Gateway :3000
            |
            +--> SiYuan Kernel :6806
            |
            +--> Redis :6379
            |
            +--> Backup Service -> /backups volume

SiYuan WebSocket /ws --> Event Bridge :3001 --> Fleet subscribers
...
The gateway proxies approved SiYuan endpoints after validating a bearer JWT. Reader, writer, backup, and admin roles are enforced in `server.js`.

| Endpoint | Method | Role | Description |
|---|---|---|---|
| `/api/health` | `GET` | public | Gateway health and upstream target. |
| `/api/notebook/lsNotebooks` | `POST` | reader | List open notebooks. |
| `/api/notebook/createNotebook` | `POST` | writer/admin | Create a notebook. |
| `/api/filetree/createDocWithMd` | `POST` | writer/admin | Create a note from Markdown. |
| `/api/storage/getRecentDocs` | `POST` | reader | Fetch recent documents. |
| `/api/search/fullTextSearchBlock` | `POST` | reader | Full-text search. |
| `/api/ref/getBacklink` | `POST` | reader/admin | Fetch backlinks for a block. |
| `/api/graph/getGraph` | `POST` | reader | Fetch graph nodes and links. |
| `/api/repo/createSnapshot` | `POST` | backup/admin | Create a repository snapshot. |
| `/api/export/exportData` | `POST` | backup/admin | Export workspace data. |
| Event bridge `/health` | `GET` | public | Event bridge status on `EVENT_BRIDGE_PORT`. |
| Event bridge `/` | `WS` | optional `EVENT_BRIDGE_AUTH_TOKEN` | Normalized note/notebook events. |
...
client = TrippMindClient("http://localhost:3000", os.environ["SIYUAN_GATEWAY_TOKEN"])
notebook = client.create_notebook("Fleet Knowledge")
note = client.create_note(notebook.id, "First Memory", "# First Memory\n\nProduction note.")
results = client.search("Production")
backlinks = client.get_backlinks(note.id)
```

```bash
curl http://localhost:3000/api/health
curl -X POST http://localhost:3000/api/storage/getRecentDocs -H "Authorization: Bearer $SIYUAN_GATEWAY_TOKEN" -H "Content-Type: application/json" -d '{"sortBy":"updated"}'
```

### Testing

Run local contract tests with `python -m pytest tests`. To run the live end-to-end integration flow, start the stack, export `RUN_LIVE_INTEGRATION=1` and `SIYUAN_GATEWAY_TOKEN`, then run `python -m pytest tests/test_integration.py`.

### Troubleshooting

| Symptom | Fix |
|---|---|
| `JWT_SECRET is required` | Set `JWT_SECRET` in `.env` and recreate `SIYUAN_GATEWAY_TOKEN` with the same secret, issuer, and audience. |
| Gateway returns `401 invalid_token` | The JWT is expired, signed with the wrong secret, or has mismatched `JWT_ISSUER`/`JWT_AUDIENCE`. |
| Gateway returns `403 endpoint_not_allowed` | Add the required role to the JWT or use an admin token. |
| SiYuan proxy calls fail | Confirm `SIYUAN_ACCESS_AUTH_CODE` and `SIYUAN_API_TOKEN` are the same value and restart `gateway` and `siyuan`. |
| Dashboard cannot connect | Confirm `DASHBOARD_CORS_ORIGIN` allows the dashboard origin and paste a valid `SIYUAN_GATEWAY_TOKEN`. |
| Event bridge health is degraded | SiYuan may still be starting or the auth code is wrong; check `docker compose logs event-bridge siyuan`. |
| Bots exit on startup | Set `TELEGRAM_BOT_TOKEN` or both `DISCORD_BOT_TOKEN` and `DISCORD_APPLICATION_ID`, then start the matching compose profile. |
| Backups do not appear | Check `BACKUP_CRON_SCHEDULE`, `SIYUAN_GATEWAY_TOKEN`, and `docker compose logs backup-service`. |

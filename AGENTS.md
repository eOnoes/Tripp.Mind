...
## 8. Tripp.Mind build status

- Phase 4 production overlay is present: Docker Compose runs SiYuan, Redis, JWT gateway, event bridge, backup service, and optional Telegram/Discord bot profiles.
- `.env.example` documents the required production variables; `.env` must keep `SIYUAN_ACCESS_AUTH_CODE` and `SIYUAN_API_TOKEN` identical.
- Phase 5 dashboard lives at `dashboard/index.html` and connects to the gateway with a JWT.
- Default validation command: `python -m pytest tests`.
- Live validation command after `docker compose up -d`: set `RUN_LIVE_INTEGRATION=1` and `SIYUAN_GATEWAY_TOKEN`, then run `python -m pytest tests/test_integration.py`.
